...
export default function Typewriter({ options }: TypewriterProps) {
  ...
  const animate = useCallback(
    (scheduleNextFrame: (waitTime: number) => void) => {
      ...
      if (isDeletingRef.current) {
        if (charIndexRef.current > 0) {
          ...
          if (loop || stringIndexRef.current < strings.length - 1) {
            stringIndexRef.current =
              (stringIndexRef.current + 1) % strings.length;
  ...
  const scheduleNextFrame = useCallback(
    (waitTime: number): void => {
      if (animationFrameRef.current) {
        cancelAnimationFrame(animationFrameRef.current);
      }
      animationFrameRef.current = requestAnimationFrame(() => {
        setTimeout(() => animate(scheduleNextFrame), waitTime);
      });
    },
    [animate],
  );
